...
def create_course_embed(info_list, subject, number):
    ...
    return_string += f"Most recently offered: {term}\n"
    return_string += f"Prerequisites: {pull_out_prereqs(info_list[5])}\n"
    return_string += f"Credit Hours: {info_list[6]}"
    return_string += "Most recently offered: " + term
...
def handle_open_command(course_code):
    ...
    try:
        ...
        logging.error(f"Failed to fetch course data: {e}")
    ...
    logging.debug(f"Created open command embed: {embed.to_dict()}")
    return {"type": 4, "data": {"embeds": [embed.to_dict()]}}
